...
class PreCalculated(unittest.TestCase):
    ...
    def test_evaluateClusters(self):
        for c12n, (clusters, expected) in enumerate(self.clusterizations):
            ...
            for e, s in se.items():
                self.assertEqual(s, expected[e],
                "Silhoette for {} in clusterization {}".format(e,c12n))

            for c, s in sc.items():
                self.assertEqual(s, self.clustersSilh[c12n][c],
                "Cluster silh for {} in clusterization {}".format(c, c12n))

            self.assertEqual(st, self.totalSilh[c12n],
                "Total silh in clusterization {}".format(c12n))
